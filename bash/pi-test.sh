...
# Check if SendPILogs is true.  If false, log and do not send the e-mail.
if [[ "$SendPILogs" = "false" ]]; then
	echo "Mailing of the log files has been disabled, no e-mail has been sent false"
fi 

if [[ "$SendPILogs" = "true" ]] && [[ "$EveryDay" = "false" ]] && [[ "$IsWeekday" = "false" ]]; then
	echo "Mailing of the log files has been disabled, no e-mail has been sent true, false, false"
fi 
