...
if [[ "$os_vers_chck" -le 107 ]]; then
	...
	vpn_status_raw=$(/usr/sbin/serveradmin command vpn:command = getConnectedUsers)
...
if [[ "$os_vers_chck" == 108 ]]; then
	...
	vpn_status_raw=$(/Applications/Server.app/Contents/ServerRoot/usr/sbin/serveradmin command vpn:command = getConnectedUsers)
fi	
...
if [[ -z "$srvr_vchck" ]]; then
	echo "$srvr_log"
		...
		# grab the raw data for AFP users, then get the usernames and IP's.
		usname="$(echo "$vpn_status_raw" | grep "name" | cut -f 2 -d = | sed 's/\"//g' | sed 's/ //g')"
		protocol="$(echo "$vpn_status_raw" | grep "protocol" | cut -f 2 -d = | sed 's/\"//g' | sed 's/ //g')"
		urip="$(echo "$vpn_status_raw" | grep -i "CommRemoteAddress" | cut -f 2 -d = | sed 's/\"//g' | sed 's/ //g')"
		ulip="$(echo "$vpn_status_raw" | grep -i "privateAddress" | cut -f 2 -d = | sed 's/\"//g' | sed 's/ //g')"
		ct="$(echo "$vpn_status_raw" | grep -i "connectionelapsedtime" | cut -f 2 -d = | sed 's/ //g' | cut -f 1 -d .)"

		# Create an array for the usernames, IP's and connection time.  Each username will match with each IP, so element 1 in usname will 	correspond with uip element 1.
		usname_array=($(echo $usname))
		prot_array=($(echo $protocol))
		urip_array=($(echo $urip))
		ulip_array=($(echo $ulip))
		ct_array=($(echo $ct))
		usname_ct="$(echo "$vpn_status_raw" | grep -i "totalconnections" | cut -f 2 -d = | sed 's/ //g')"

		# Put it together and make it pretty.
		echo "The VPN Service currently has "$usname_ct" users connected"
		echo ""
		if [[ "$usname_ct" -gt 0 ]]; then
			echo -e "Username"'\t''\t'"local IP"'\t''\t'"Remote IP"'\t''\t'"Protocol"'\t'"Connected For"
			echo "-------------------------------------------------------------------------------------------------------------------"
			# Number of items in the username array, so we know how many times to iterate through
			# the count of items in each array should match - might add checking in later versions
			# Iterate through each array - printing the username then corresponding IP.  
			...
			for ((i=0;i<$usname_ct;i++)); do
					...
					cseconds=$(($cdiff-($cminutes*60)))
				if [[ ${#usname_array[${i}]} -ge 8 ]]; then	
					echo -e "${usname_array[${i}]}"'\t'"${ulip_array[${i}]}"'\t''\t'"${urip_array[${i}]}"'\t''\t'"${prot_array[${i}]}"'\t''\t'`format "$cdays" "day"` `format "$chours" "hour"` `format "$cminutes" "minute"`
					echo "-------------------------------------------------------------------------------------------------------------------"
				else
					echo -e "${usname_array[${i}]}"'\t''\t''\t'"${ulip_array[${i}]}"'\t''\t'"${urip_array[${i}]}"'\t''\t'"${prot_array[${i}]}"'\t''\t'`format "$cdays" "day"` `format "$chours" "hour"` `format "$cminutes" "minute"`
					echo "-------------------------------------------------------------------------------------------------------------------"
			...
			echo "There are no users connected via VPN"
		fi
	else 
		echo "$srvr_log"
fi
